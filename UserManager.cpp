...
void UserManager::saveUsersToFile(SystemContext &ctx)
{
    ...
    // 遍历 UserList 中的每一个用户
    for (const auto &user : ctx.uList.users)
    {
        outFile << user.userId << "\n";
    }
    outFile.close();
...
void UserManager::loadUsers(SystemContext &ctx)
{
    ...
    ctx.uList.users.clear(); // 先清空当前列表，防止重复
    int id;
    while (inFile >> id)
    {
        User tempUser;
        tempUser.userId = id;
        ctx.uList.users.push_back(tempUser);
    }
    // while (inFile >> id >> age) { ... }
    inFile.close();
}
...
void UserManager::SwitchUser(SystemContext &ctx, const std::vector<std::string> &args)
{
    ...
    // 1. 在 uList 中查找用户是否存在
    auto it = std::find_if(ctx.uList.users.begin(), ctx.uList.users.end(),
                           [targetId](const User &u)
                           {
                               return u.userId == targetId;
                           });
    // 2. 如果没找到，则添加到列表中
    if (it == ctx.uList.users.end())
    {
